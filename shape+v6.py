...
def get_foul_value(description):
...
pitcher_pitch_data['foul_value'] = pitcher_pitch_data.apply(lambda x: get_foul_value(x.description), axis = 1)
...
y_cols = ['swstr_value', 'foul_value', 'gb_value', 'ab_value']
...
foul_rate = sh_shape_grades.foul
gb_rate = sh_shape_grades.gb
ab_rate = sh_shape_grades.ab
xRV = swstr_rv*swstr_rate + foul_rv*foul_rate + gb_rv*gb_rate + ab_rv*ab_rate
...
foul_rate = oh_shape_grades.foul
...
def grade_repertoire(pitcher, verbose = True, backend = 'qt'):
    ...
    if hand == 'R':
        RHB_percent = round(df.platoon_percent.values[0], 3)
    else:
        RHB_percent = round(1 - df.platoon_percent.values[0], 3)
    ...
    total_shape = round(RHB_shape*RHB_percent + LHB_shape*(1 - RHB_percent))
    total_row = ['Total', '', '', '', RHB_percent*100, RHB_shape, (1 - RHB_percent)*100, LHB_shape]
...
sh_shape_grades['weighted_grade'] = sh_shape_grades['Shape+'] * sh_shape_grades['percent'] / 100
sh_pitcher_grades = sh_shape_grades.groupby('player_name').apply(
    lambda x: pd.Series({
        'aggregate_grade': round(x['weighted_grade'].sum() / x['percent'].sum() * 100, 1)
    })).reset_index()
...
oh_shape_grades['weighted_grade'] = oh_shape_grades['Shape+'] * oh_shape_grades['percent'] / 100
oh_pitcher_grades = oh_shape_grades.groupby('player_name').apply(
    lambda x: pd.Series({
        'aggregate_grade': round(x['weighted_grade'].sum() / x['percent'].sum() * 100, 1)
    })).reset_index()
oh_pitcher_grades['platoon_percent'] = round(1 - oh_pitcher_grades.apply(lambda x: classified_pitch_data.query('player_name == @x.player_name and game_year == 2024')['platoon_percent'].iloc[0], axis = 1), 3)
...
grade_repertoire('Hoffman, Jeff', verbose=False)
